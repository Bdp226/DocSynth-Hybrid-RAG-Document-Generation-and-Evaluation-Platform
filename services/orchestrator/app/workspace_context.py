...
_NOISE_LINE_PATTERNS = [
    ...
    # PowerPoint template instructions aimed at the deck author, never at the reader.
    re.compile(r"^this\s+slide\s+template\s+contains\b.*$", re.IGNORECASE),
    re.compile(r"^.*\bdo\s+not\s+ungroup\s+any\s+elements\b.*$", re.IGNORECASE),
    re.compile(r"^.*\banimations?\s*>\s*animation\s+pane\b.*$", re.IGNORECASE),
    # Corporate template placeholders: filler copy, style specimens and classification stamps.
    re.compile(r"^.*\blorem\s+ipsum\b.*$", re.IGNORECASE),
    re.compile(r"^.*\bsh-[a-z]+-headline\b.*$", re.IGNORECASE),
    re.compile(r"^restricted\s*(\u00a9|\(c\)).*$", re.IGNORECASE),
    re.compile(r"^.*\brestricted\s+access\s+to\s+llms?\b.*$", re.IGNORECASE),
    re.compile(r"^.*\brestricted\s+(?:\u00a9|\(c\)|confidential)\b.*$", re.IGNORECASE),
    # First-person drafting notes the author left for reviewers.
    re.compile(r"^a\s+note\s+on\s+accuracy\b.*$", re.IGNORECASE),
]
...
_MEETING_CHATTER_RE = re.compile(
    ...
    r"(?:discussed|introduced|asked|explained|mentioned|confirmed|stated|noted|outlined"
    r"|advised|agreed|suggested|offered|estimated|emphasi[sz]ed|highlighted|reported)\b",
)
...
def _looks_like_noise_line(line: str) -> bool:
    ...
    if len(stripped) <= _MAX_CHATTER_LINE_CHARS and _MEETING_CHATTER_RE.search(stripped):
        return True
